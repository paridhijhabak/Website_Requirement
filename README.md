...
TechBird-branded frontend (`index.html`), file uploads, storage, and an admin dashboard
...
```
index.html          the public form (client-facing, TechBird branded)
...
Logos are inlined as base64 inside `index.html` and `admin.html`, so there is no
separate `assets/` folder to keep track of. These two files work standalone.

## 1. Push this folder to GitHub

Create a new repo and push these files as-is (keep the `api/` folder at the root,
alongside `index.html`, `admin.html`, and `package.json`).
...
- Deploy once. The form UI will work immediately, but submissions and uploads will
  fail until storage is connected (next step).
...
There is no email-based password reset, it's a single value you control. To change
it later, edit the same environment variable and redeploy. `admin.html` has a
"Forgot password?" link that explains this same process.
...
- **File size:** capped at 4MB per file, enforced both client-side and in
  `api/upload.js` (Vercel's serverless functions have a request body limit, so very
  large brand files won't upload, ask the client to compress or share large files
  by link instead, e.g. Google Drive).
